...
/// </remarks>
public partial class HamkareExitPrompt : HamkareComponentBase, IAsyncDisposable
{
    ...
    /// </summary>
    /// <remarks>
    /// When <c>false</c>, navigation inside the app uses a HamkareBlazor message box with <see cref="Title"/> and <see cref="Text"/>.
    ...
    private string TitleToDisplay => Title ?? Localizer[LanguageResource.HamkareExitPrompt_Title];

    private string TextToDisplay => Text ?? Localizer[LanguageResource.HamkareExitPrompt_Text];

    public HamkareExitPrompt()
    {
    ...
    private async Task<bool> IsNavigationAllowedAsync()
    {
        ...
        if (UseNativePrompt)
        {
            return await JsRuntime.InvokeAsync<bool>("hamkareExitPrompt.handleBeforeNavigation", _promptId);
        }

        // Otherwise, use the HamkareBlazor confirmation dialog and allow navigation only on explicit confirmation.
        return await DialogService.ShowMessageBoxAsync(
            TitleToDisplay,
            TextToDisplay,
            Localizer[LanguageResource.HamkareExitPrompt_Exit],
            Localizer[LanguageResource.HamkareExitPrompt_Cancel]
        ) == true;
    }
    ...
    private async Task EnableAsync()
    {
        ...
        var success = await JsRuntime.InvokeVoidAsyncWithErrorHandling("hamkareExitPrompt.enable", _promptId, TextToDisplay);
        if (!success)
        {
            throw new InvalidOperationException($"Failed to enable {nameof(HamkareExitPrompt)} JS interop for prompt '{_promptId}'.");
        }
    }
    ...
    private async Task SetTextAsync()
    {
        ...
        await JsRuntime.InvokeVoidAsyncWithErrorHandling("hamkareExitPrompt.setText", _promptId, TextToDisplay);
    }
    ...
    private async Task DisableAsync()
    {
        ...
        await JsRuntime.InvokeVoidAsyncWithErrorHandling("hamkareExitPrompt.disable", _promptId);
    }
