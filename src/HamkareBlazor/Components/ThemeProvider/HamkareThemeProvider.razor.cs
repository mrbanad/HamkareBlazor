...
partial class HamkareThemeProvider : ComponentBaseWithState, IAsyncDisposable
{
    ...
    private readonly HamkareTheme _originalHamkareTheme = new();
    ...
    protected string BuildTheme()
    {
        ...
        themeStringBuilder.AppendLine("<style class='hamkare-theme-provider'>");
    ...
    protected virtual void GenerateTheme(StringBuilder themeStringBuilder)
    {
        ...
        themeStringBuilder.AppendLine($"--{Palette}-surface-rgb: {palette.Surface.ToString(HamkareColorOutputFormats.ColorElements)};");
        ...
        themeStringBuilder.AppendLine($"--hamkare-native-html-color-scheme: {(_isDarkModeState.Value ? "dark" : "light")};");
    }

    protected HamkareTheme GetTheme() => Theme ?? _originalHamkareTheme;
