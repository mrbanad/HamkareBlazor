...
namespace HamkareBlazor
{
    ...
    public partial class HamkareFileUpload<T> : HamkareFormComponent<T, string>
    {
        ...
        protected string DragClass =>
            new CssBuilder("hamkare-file-upload-dragarea")
                .AddClass("hamkare-file-upload-dragarea-clickable", !GetDisabledState())
                .AddClass("hamkare-border-primary", _draggingState.Value)
                .Build();

        protected string InputClasses =>
            new CssBuilder(InputClass)
                .AddClass("hamkare-file-upload-dragover", DragAndDrop && (Hidden || CustomContent == null))
                .Build();
        ...
        /// </summary>
        /// <remarks>
        /// The context is the current <see cref="HamkareFileUpload{T}"/> instance and can be used to call <see cref="OpenFilePickerAsync"/>.
        /// </remarks>
        [Parameter]
        [Category(CategoryTypes.FileUpload.Behavior)]
        public RenderFragment<HamkareFileUpload<T>>? CustomContent { get; set; }
        ...
        private async Task ProcessFileChangeAsync(InputFileChangeEventArgs args)
        {
            ...
            if (args.FileCount > MaximumFileCount)
            {
                ...
                _validationErrors.Add(Localizer[LanguageResource.HamkareFileUpload_MaximumFileCountExceeded, args.FileCount, MaximumFileCount]);
