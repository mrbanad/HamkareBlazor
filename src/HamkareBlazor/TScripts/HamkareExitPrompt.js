...
/**
 * Navigation/exit prompt interop for the HamkareExitPrompt component.
 * Keeps browser unload protection in JS where `beforeunload` is handled natively.
 */
class HamkareExitPrompt {
    constructor() {
    ...
    _throwPromptNotFound(id) {
        throw new Error(`[HamkareBlazor] HamkareExitPrompt: Prompt with id '${id}' is not registered.`);
    }
...
if (!window.hamkareExitPrompt) {
    window.hamkareExitPrompt = new HamkareExitPrompt();
}
