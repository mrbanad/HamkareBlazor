...
/**
 * Auto-grow textarea sizing logic for the HamkareInput component.
...
window.hamkareInputSizing = {
    ...
    init: (elem, maxLines) => {
        ...
        elem.adjustSizingHeight = function (didReflow = false) {
            ...
            const scrollStates = window.hamkareInputSizing.captureScrollStates(elem);

            // Auto mode - grow/shrink based on content
            elem.style.height = 0;

            if (didReflow) {
                const previousTextAlign = elem._hamkareInputSizingPreviousTextAlign;
                elem.style.textAlign = previousTextAlign !== undefined ? previousTextAlign : '';
                delete elem._hamkareInputSizingPreviousTextAlign;
            }

            // Styles can change at runtime (variant/margin/typography/classes), so always recalculate metrics.
            const metrics = window.hamkareInputSizing.computeMetrics(elem);
            ...
            window.hamkareInputSizing.restoreScrollStates(scrollStates);
            ...
            if (!didReflow && initialOverflowY !== elem.style.overflowY && elem.style.overflowY === 'hidden') {
                elem._hamkareInputSizingPreviousTextAlign = elem.style.textAlign;
